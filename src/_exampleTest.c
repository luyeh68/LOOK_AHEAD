...
state_t do_init(state_data_t *data) {
  ...
  //next_state = STATE_RUN; --- NOT ALLOWED!!! ---
...
// 1. from init to idle
// 2. from run to idle
void reset(state_data_t *data) //resetting the counter every time we go back into IDLE
{
...
// FUNCTION to be called for running my machine
...
int main() {
  ...
  do {
    cur_state = run_state(cur_state, &data); //instead of &data it's &state where //state_data_t state =  {.count_idle = 0, . count_run = 0} is NOT COMMENTED (SAME below); run_state returns the next state to which I want to transition
    sleep(1);
  } while (cur_state != STATE_STOP);
  run_state(cur_state, &data); //instead of &data it's inserted &state
  return 0;
}
#endif
